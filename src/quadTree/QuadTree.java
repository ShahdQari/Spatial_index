...
public class QuadTree{
	...
	public Rectangle[] packInRectangles(
      ...
      public Rectangle[] StoreRectanglesToArrayText(final Point[] sample) throws IOException {
		for(Point p : sample){
		  this.insert(p);
		}
		result = new ArrayList<Rectangle>();
		OutputStreamWriter writer = new OutputStreamWriter(
				new FileOutputStream("C:\\Users\\DELL\\Desktop\\University\\graduation project2\\quadtree_mbrs.txt", false), "UTF-8");
		writer.write("MBR\n");
                printLeafNodes(this);
		writer.close();
		Rectangle[] cellinfo = new Rectangle[result.size()];
